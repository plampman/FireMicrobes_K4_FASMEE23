...
PA_stats_k4 <- PA_stats_k4 %>%
  mutate(
    SmokeLevel = case_when(
      ...
      TRUE ~ NA_character_),
    SmokeLevel2 = case_when(
      MedianPM2.5_ug.m3 < 20 ~ "None",
      MedianPM2.5_ug.m3 >= 20 & MedianPM2.5_ug.m3 < 500 ~ "Low",
      MedianPM2.5_ug.m3 >= 500 ~ "High",
      TRUE ~ NA_character_),
    SmokeLevel = factor(SmokeLevel, levels = c("None", "Low", "Moderate", "High")),
    SmokeLevel2 = factor(SmokeLevel2, levels = c("None", "Low", "High"))
    )
...
stats_k4 <- samples_k4 %>%
  group_by(Sample, Start_DateTime_cdt, Stop_DateTime_cdt, experiment) %>%
  dplyr::summarise(
