#########################################################################
###
### Program name: Konza 4 Raw Flight Log Wrangling
###
### Purpose: Adding HAGL and combining K4 flights into one file
###
...
blue_AirData <- list.files('./Input_Data/Konza4/FlightLogs/AirData_Konza4/', pattern = 'csv$', full.names = T, recursive = T) %>%
  map(~read.csv(.x)) %>%
  bind_rows()
...
# 
# write.csv(blue_GL, './Input_Data/Konza4/FlightLogs/blueGL.csv')

##################################################################################################

blue_GL <- read.csv('./Input_Data/Konza4/FlightLogs/blueGL.csv', header = F) 
...
blue_flight_samples <- blue_AirData %>%
  mutate(int = findInterval(blue_AirData$datetime_cdt, leland_ints_seq$time_cdt)) %>%
  filter(int %% 2 == 1)

blue_flight_samples <- left_join(blue_flight_samples, leland_ints, by = "int")

write.csv(blue_flight_samples, './Output/Output_data/K4/K4_FlightsAGL.csv', row.names = F)
